...
namespace thomasmars\LaravelH5p\Repositories;

use Illuminate\Support\Facades\App;
use H5PFrameworkInterface;
use thomasmars\LaravelH5p\Helpers\H5pHelper;
...
use thomasmars\LaravelH5p\Eloquents\H5pLibrariesLibrary;
use thomasmars\LaravelH5p\Eloquents\H5pContent;
use thomasmars\LaravelH5p\Eloquents\H5pContentsLibrary;
use thomasmars\LaravelH5p\Eloquents\H5pResult;
use thomasmars\LaravelH5p\Eloquents\H5pContentsUserData;
    ...
    public function fetchExternalData($url, $data = NULL, $blocking = TRUE, $stream = NULL) {
        ...
        try {
            if ($data !== NULL) {
                // Post
                $options['body'] = $data;
                $response = $client->request("POST", $url, array('form_params' => $options));
            } else {
                // Get
